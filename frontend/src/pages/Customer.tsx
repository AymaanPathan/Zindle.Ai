...
const css = `
  ...
  .cp-in { animation: cp-in 0.2s ease both; }
  ...
  .cp-compose {
    ...
    width: 500px; background: var(--w);
    border-radius: 10px 10px 0 0;
    border: 1px solid var(--g200);
    box-shadow: 0 8px 40px rgba(0,0,0,0.12), 0 2px 8px rgba(0,0,0,0.06);
    display: flex; flex-direction: column;
    z-index: 100; animation: cp-in 0.18s ease both; overflow: hidden;
  }
  .cp-compose-header {
  ...
  .cp-compose-label {
    font-size: 11.5px; color: var(--g400); width: 58px; flex-shrink: 0;
  ...
  .cp-compose-body {
    ...
    font-family: var(--sans); letter-spacing: -0.01em;
    resize: none; line-height: 1.7; min-height: 180px;
  }
  ...
  .cp-risk-card { background: #fff; border: 1px solid #e5e7eb; border-radius: 12px; overflow: hidden; }
  .cp-risk-card-header { display: flex; align-items: center; justify-content: space-between; padding: 12px 18px; border-bottom: 1px solid #f3f4f6; }
  .cp-signal-row { display: flex; align-items: center; justify-content: space-between; padding: 10px 18px; border-bottom: 1px solid #f3f4f6; gap: 12px; }
...
function Spinner({ size = 14 }: { size?: number }) {
  return <div style={{ width: size, height: size, border: "1.5px solid #e5e7eb", borderTopColor: "#6b7280", borderRadius: "50%", flexShrink: 0, animation: "cp-spin 0.6s linear infinite" }} />;
}
  ...
  return (
    <div className="cp-score-ring">
      <svg width="68" height="68" viewBox="0 0 68 68" style={{ transform: "rotate(-90deg)" }}>
        <circle cx="34" cy="34" r={r} fill="none" stroke="#f3f4f6" strokeWidth="4.5" />
  ...
  return (
    <div className="cp-risk-card">
      <div className="cp-risk-card-header">
        <span style={{ fontSize: 12.5, fontWeight: 600, color: "#111827", letterSpacing: "-0.02em" }}>{title}</span>
      ...
      {rows.map((row, i) => (
        <div className="cp-signal-row" key={i}>
          ...
          <span style={{ fontSize: 12.5, fontWeight: 500, color: row.flag ? "#ef4444" : "#111827", letterSpacing: "-0.015em", textAlign: "right" as const }}>
            {row.value}
          </span>
        </div>
      ))}
...
function RiskSkeleton() {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14, padding: "20px 0" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 16, padding: "20px", background: "#fff", border: "1px solid #e5e7eb", borderRadius: 12 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", background: "#fff", border: "1px solid #e5e7eb", borderRadius: 12, overflow: "hidden" }}>
        {[0,1,2,3].map(i => (
          <div key={i} style={{ padding: "18px 20px", borderRight: i < 3 ? "1px solid #f3f4f6" : "none" }}>
...
function ComposeWindow({ email, onClose }: { email: string; onClose: () => void }) {
  ...
  useEffect(() => {
    if (sendSuccess) { const t = setTimeout(() => { dispatch(clearSendStatus()); onClose(); }, 2500); return () => clearTimeout(t); }
  }, [sendSuccess, dispatch, onClose]);

  const handleGenerate = () => { dispatch(draftCustomerEmail({ email, type: selectedType, customInstruction: selectedType === "custom" ? customInstruction : undefined })); setConfirmSend(false); };
  const handleSend     = () => { dispatch(sendCustomerEmail({ email, subject: editSubject, body: editBody })); setConfirmSend(false); };

  return (
    <div className="cp-compose" style={{ height: minimized ? "auto" : 580 }}>
      <div className="cp-compose-header" onClick={() => setMinimized(m => !m)}>
        <span style={{ fontSize: 12.5, fontWeight: 500 }}>{draft ? `Re: ${editSubject || "Draft"}` : "New Message"}</span>
        <div style={{ display: "flex", alignItems: "center", gap: 1 }}>
          {[
            { d: minimized ? "M2 8L8 2" : "M2 8h6", d2: !minimized ? "M8 8V2" : null, fn: () => setMinimized(m => !m) },
            { d: "M2 2l6 6M8 2L2 8", d2: null, fn: onClose },
          ].map((btn, i) => (
            <button key={i} onClick={e => { e.stopPropagation(); btn.fn(); }} style={{ width: 22, height: 22, border: "none", background: "transparent", cursor: "pointer", color: "rgba(255,255,255,0.65)", borderRadius: 3, display: "flex", alignItems: "center", justifyContent: "center" }}
              onMouseEnter={e => e.currentTarget.style.background = "rgba(255,255,255,0.12)"}
              onMouseLeave={e => e.currentTarget.style.background = "transparent"}
            >
              <svg width="10" height="10" viewBox="0 0 10 10" fill="none">
                <path d={btn.d} stroke="currentColor" strokeWidth="1.4" strokeLinecap="round"/>
                {btn.d2 && <path d={btn.d2} stroke="currentColor" strokeWidth="1.4" strokeLinecap="round"/>}
              </svg>
            </button>
          ))}
        </div>
        ...
        <>
          <div style={{ borderBottom: "1px solid #e5e7eb" }}>
            <div className="cp-compose-field">
              ...
              <span style={{ fontSize: 13, color: "#111827", padding: "10px 14px 10px 0", flex: 1 }}>{email}</span>
            </div>
            <div className="cp-compose-field">
          ...
          <textarea className="cp-compose-body" placeholder="Write your message, or generate an AI draft below…" value={editBody} onChange={e => setEditBody(e.target.value)} style={{ flexShrink: 0 }} />
          <div style={{ borderTop: "1px solid #e5e7eb", padding: "10px 14px", background: "#f9fafb", display: "flex", flexDirection: "column", gap: 8 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 5, flexWrap: "wrap" as const }}>
              <span style={{ fontSize: 11, color: "#9ca3af", marginRight: 2 }}>Draft type:</span>
              {DRAFT_TYPES.map(t => (
                <button key={t.id} className={`cp-dtype${selectedType === t.id ? " active" : ""}`} onClick={() => setSelectedType(t.id)}>{t.label}</button>
              ))}
            </div>
            {selectedType === "custom" && (
              <input className="cp-compose-input" style={{ border: "1px solid #e5e7eb", borderRadius: 6, padding: "7px 10px", fontSize: 12 }}
                placeholder="Describe what you want…" value={customInstruction} onChange={e => setCustomInstruction(e.target.value)} />
            )}
            {draftError && <div style={{ fontSize: 11.5, color: "#b91c1c", padding: "6px 10px", background: "#fef2f2", borderRadius: 5, border: "1px solid #fecaca" }}>{draftError}</div>}
            {sendError  && <div style={{ fontSize: 11.5, color: "#b91c1c", padding: "6px 10px", background: "#fef2f2", borderRadius: 5, border: "1px solid #fecaca" }}>{sendError}</div>}
            {sendSuccess && (
              <div style={{ fontSize: 12, color: "#15803d", padding: "7px 10px", background: "#f0fdf4", borderRadius: 5, border: "1px solid #d1fae5", display: "flex", alignItems: "center", gap: 6 }}>
                <svg width="11" height="11" viewBox="0 0 14 14" fill="none"><path d="M2.5 7.5L5.5 10.5L11.5 4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
                Sent{sentMessageId ? ` · ${sentMessageId.slice(0, 14)}…` : ""}
              </div>
            )}
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
              <div style={{ display: "flex", gap: 6 }}>
                <button onClick={handleGenerate} disabled={draftLoading} style={{ display: "inline-flex", alignItems: "center", gap: 5, padding: "6px 13px", borderRadius: 6, border: "none", background: draftLoading ? "#f3f4f6" : "#111827", color: draftLoading ? "#9ca3af" : "#fff", fontSize: 12.5, fontWeight: 500, cursor: draftLoading ? "not-allowed" : "pointer", transition: "opacity 0.1s" }}>
                  {draftLoading ? <><Spinner size={11} /> Generating…</> : "Generate draft"}
                </button>
                {(editSubject || editBody) && (
                  <button onClick={() => navigator.clipboard.writeText(`Subject: ${editSubject}\n\n${editBody}`)} style={{ padding: "6px 11px", borderRadius: 6, border: "1px solid #e5e7eb", background: "#fff", fontSize: 12.5, color: "#6b7280", cursor: "pointer" }}>
                    Copy
                  </button>
                )}
              </div>
              {!confirmSend ? (
                <button onClick={() => setConfirmSend(true)} disabled={!editSubject || !editBody || sendLoading} style={{ padding: "6px 16px", borderRadius: 6, border: "none", background: (!editSubject || !editBody) ? "#e5e7eb" : "#1a73e8", color: (!editSubject || !editBody) ? "#9ca3af" : "#fff", fontSize: 13, fontWeight: 600, cursor: (!editSubject || !editBody) ? "not-allowed" : "pointer", transition: "opacity 0.1s", display: "flex", alignItems: "center", gap: 5 }}>
                  {sendLoading ? <><Spinner size={11} /> Sending…</> : "Send"}
                </button>
              ) : (
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  <span style={{ fontSize: 12, color: "#374151" }}>Send to <strong>{email.split("@")[0]}</strong>?</span>
                  <button onClick={() => setConfirmSend(false)} style={{ padding: "4px 9px", borderRadius: 5, border: "1px solid #e5e7eb", background: "#fff", fontSize: 12, color: "#6b7280", cursor: "pointer" }}>Cancel</button>
                  <button onClick={handleSend} disabled={sendLoading} style={{ padding: "4px 12px", borderRadius: 5, border: "none", background: "#1a73e8", color: "#fff", fontSize: 12, fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", gap: 4 }}>
                    {sendLoading ? <><Spinner size={10} /> Sending…</> : "Confirm"}
                  </button>
                </div>
              )}
            </div>
  ...
  return (
    <div style={{ overflowX: "auto" as const }}>
      <table className="cp-tbl">
        <thead><tr>{["Invoice","Status","Amount due","Paid","Remaining","Due date",""].map(h => <th key={h}>{h}</th>)}</tr></thead>
        <tbody>
          {invoices.map(inv => {
            ...
            return (
              <tr key={inv.id}>
                ...
                <td>
                  <span style={{ display: "inline-flex", alignItems: "center", gap: 4, fontSize: 11, fontWeight: 500, padding: "2px 7px", borderRadius: 4, background: s.bg, color: s.color, border: `1px solid ${s.border}` }}>{inv.status}</span>
                  {inv.is_overdue && <div style={{ fontSize: 10.5, color: "#b91c1c", marginTop: 2, fontWeight: 500 }}>{inv.days_overdue}d overdue</div>}
                </td>
                <td><span style={{ fontSize: 13, fontWeight: 600, color: "#111827", fontVariantNumeric: "tabular-nums" }}>{fmt(inv.amount_due, currency)}</span></td>
                ...
                <td style={{ textAlign: "right" as const }}>
                  <div style={{ display: "flex", gap: 5, justifyContent: "flex-end" }}>
                    {inv.hosted_invoice_url && <a href={inv.hosted_invoice_url} target="_blank" rel="noopener noreferrer" className="cp-inv-btn" style={{ background: "#111827", color: "#fff", border: "1px solid #111827" }}>Pay ↗</a>}
                    {inv.invoice_pdf && <a href={inv.invoice_pdf} target="_blank" rel="noopener noreferrer" className="cp-inv-btn" style={{ background: "#fff", color: "#6b7280", border: "1px solid #e5e7eb" }}>PDF ↗</a>}
                  </div>
                </td>
              </tr>
            );
      ...
      {events.map((ev, i) => {
        ...
        return (
          <div key={i} className="cp-ev">
            <div style={{ width: 26, height: 26, borderRadius: 7, background: "#f9fafb", border: "1px solid #e5e7eb", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>{icon}</div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  <span style={{ fontSize: 12.5, fontWeight: 500, color: "#111827", letterSpacing: "-0.015em" }}>{label}</span>
                  <span style={{ fontSize: 10, fontWeight: 500, padding: "1px 5px", borderRadius: 3, border: "1px solid #e5e7eb", color: src.color, background: "#fff" }}>{src.label}</span>
                </div>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 12, padding: "20px 0" }}>
      ...
      <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 12, overflow: "hidden" }}>
        ...
        <div style={{ padding: "20px 24px", display: "flex", alignItems: "center", gap: 20 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 16, fontWeight: 600, color: "#111827", letterSpacing: "-0.025em", lineHeight: 1.2 }}>{riskDetail.name || riskDetail.email}</div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", borderTop: "1px solid #f3f4f6" }}>
          {[
            { label: "Amount due",    value: totalAmountDue > 0 ? fmtK(totalAmountDue) : "₹0", sub: totalAmountDue > 0 ? "outstanding" : "all clear", color: totalAmountDue > 0 ? "#111827" : "#16a34a", flag: totalAmountDue > 50000 },
            ...
            <div key={label} className="cp-kpi-tile" style={{ borderRight: i < 3 ? "1px solid #f3f4f6" : "none" }}>
      ...
      {riskDetail.breakdown?.length > 0 && (
        <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 12, overflow: "hidden" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 18px", borderBottom: "1px solid #f3f4f6" }}>
            <span style={{ fontSize: 12.5, fontWeight: 600, color: "#111827", letterSpacing: "-0.02em" }}>Score breakdown</span>
            <span style={{ fontSize: 11, color: "#6b7280", background: "#f3f4f6", border: "1px solid #e5e7eb", padding: "2px 8px", borderRadius: 4 }}>{riskDetail.breakdown.length} signals</span>
          </div>
          {riskDetail.breakdown.slice().sort((a: any, b: any) => b.points - a.points).map((item: any, i: number) => {
            ...
            return (
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 12.5, fontWeight: 500, color: "#111827", letterSpacing: "-0.015em" }}>{item.signal}</div>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 7, width: 90 }}>
                  <div style={{ flex: 1, height: 3, borderRadius: 99, background: "#f3f4f6", overflow: "hidden" }}>
...
export default function CustomerProfile() {
  ...
  return (
    <div className="cp">
      ...
      {/* Top bar */}
      <div style={{
        height: 48, borderBottom: "1px solid #e5e7eb",
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, minWidth: 0 }}>
          <div style={{ width: 26, height: 26, borderRadius: 7, background: "#f3f4f6", border: "1px solid #e5e7eb", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, fontWeight: 600, color: "#374151", flexShrink: 0 }}>
            {inits(name, email)}
          </div>
          <div style={{ minWidth: 0 }}>
            <div style={{ fontSize: 13, fontWeight: 500, color: "#111827", letterSpacing: "-0.02em", lineHeight: 1.2, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" as const }}>{name}</div>
            {contact?.company && <div style={{ fontSize: 11, color: "#9ca3af" }}>{contact.company}</div>}
          </div>
        </div>

        {summary && (
          <span style={{ display: "inline-flex", alignItems: "center", gap: 4, fontSize: 11, fontWeight: 500, padding: "2px 8px", borderRadius: 4, background: risk.bg, color: risk.color, border: `1px solid ${risk.border}`, letterSpacing: "-0.01em", flexShrink: 0 }}>
        ...
        {riskDetail && (
          <span style={{ display: "inline-flex", alignItems: "center", gap: 4, fontSize: 11, fontWeight: 600, padding: "2px 8px", borderRadius: 4, background: "#eff6ff", color: "#1d4ed8", border: "1px solid #bfdbfe", flexShrink: 0 }}>
        ...
        <button onClick={() => setComposeOpen(true)} style={{
          ...
          height: 30, padding: "0 13px", borderRadius: 6, border: "none",
          background: "#111827", color: "#fff", fontSize: 12.5, fontWeight: 500,
          cursor: "pointer", letterSpacing: "-0.01em", transition: "opacity 0.1s",
        }}
      ...
      <div className="cp-scroll" style={{ flex: 1, overflowY: "auto", padding: "24px 24px 80px" }}>
        ...
        {profileError && !profileLoading && (
          <div style={{ padding: "10px 14px", borderRadius: 7, background: "#fef2f2", border: "1px solid #fecaca", fontSize: 13, color: "#b91c1c" }}>{profileError}</div>
        )}

        {profile && !profileLoading && (
          <div className="cp-in" style={{ display: "flex", flexDirection: "column", gap: 20 }}>

            {/* Stat cards — proper SVG icons, not emoji */}
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 10 }}>
              {[
                ...
                <div key={label} style={{ border: "1px solid #e5e7eb", borderRadius: 10, padding: "16px 18px", background: "#fff", transition: "border-color 0.12s" }}
                ...
                >
                  ...
                  <div style={{ fontFamily: "'Instrument Serif',Georgia,serif", fontSize: 28, fontWeight: 400, color: "#111827", lineHeight: 1, letterSpacing: "-0.02em" }}>{val}</div>
              ...
              <div style={{ display: "flex", alignItems: "center", borderBottom: "1px solid #e5e7eb", padding: "0 20px" }}>
                {([
                  ...
                  <button key={t.id} className={`cp-tab${tab === t.id ? " active" : ""}`} onClick={() => setTab(t.id)}>
                    {t.isAI && (
                      <span style={{ fontSize: 8.5, fontWeight: 700, letterSpacing: "0.06em", color: tab === t.id ? "#1d4ed8" : "#9ca3af", background: tab === t.id ? "#eff6ff" : "#f3f4f6", border: `1px solid ${tab === t.id ? "#bfdbfe" : "#e5e7eb"}`, padding: "1px 4px", borderRadius: 3, textTransform: "uppercase" as const }}>AI</span>
                    )}
                    {t.isAI ? (riskDetail ? `Risk · ${riskDetail.score}` : "Risk Intel") : t.label}
                    {t.count !== undefined && (
                      <span style={{ fontSize: 10.5, fontWeight: 500, color: tab === t.id ? "#374151" : "#9ca3af", background: tab === t.id ? "#f3f4f6" : "transparent", border: `1px solid ${tab === t.id ? "#e5e7eb" : "transparent"}`, padding: "0 5px", borderRadius: 3, lineHeight: "16px" }}>
                        {t.count}
                      </span>
                    )}
                  </button>
                ...
                <div style={{ padding: "20px" }}>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                    {[
                      ...
                      <div key={label} style={{ padding: "12px 14px", borderRadius: 8, border: "1px solid #e5e7eb", background: "#f9fafb" }}>
                        ...
                        <div style={{ fontSize: 13, fontWeight: 500, color: "#111827", letterSpacing: "-0.015em" }}>{value}</div>
                      </div>
                    ))}
                  </div>
                </div>
